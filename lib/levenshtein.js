...
export function compute(stringA: string, stringB: string): [number, Array<number>] {
  const matrix = [];
  ...
  if (stringA == stringB) {
    ...
    return [stringB.length, matrix];
  }
  else if (stringB == '') {
    return [stringA.length, matrix];
  }
  ...
  return [current[current.length - 1], matrix];
}
...
export default class Levenshtein {
  ...
  constructor(stringA, stringB) {
    const [distance, matrix] = compute(stringA, stringB);
